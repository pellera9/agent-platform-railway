...
- **Schedules** — what `register_schedules()` ([`app/schedules.py`](../../../app/schedules.py)) registers, and the env that gates each (e.g. the usage rollup arms on `SLACK_BOT_TOKEN` + `SLACK_CHANNEL`). Every schedule `endpoint` should map to a real workflow `id`.
